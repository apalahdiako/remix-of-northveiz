...
const Checkout = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Use the checkout_order function for atomic stock check + order creation
      ...
      clearCart();
      navigate(`/payment?orderId=${orderId}&orderNumber=${orderNumber}`);
    } catch (error: any) {
  ...
  return (
    <div className="container px-4 py-6 pt-24 max-w-2xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          <RadioGroup value={formData.paymentMethod} onValueChange={(value) => setFormData({ ...formData, paymentMethod: value })}>
            <div className="flex items-center space-x-2 border rounded-lg p-4">
              ...
              <Label htmlFor="bank_transfer" className="flex-1 cursor-pointer">Transfer Bank</Label>
            </div>
            <div className="flex items-center space-x-2 border rounded-lg p-4">
              <RadioGroupItem value="credit_card" id="credit_card" />
              <Label htmlFor="credit_card" className="flex-1 cursor-pointer">Kartu Kredit/Debit</Label>
            </div>
            <div className="flex items-center space-x-2 border rounded-lg p-4">
              <RadioGroupItem value="ewallet" id="ewallet" />
              <Label htmlFor="ewallet" className="flex-1 cursor-pointer">e-Wallet</Label>
            </div>
          </RadioGroup>
        </div>
        ...
        <Button type="submit" className="w-full h-14 rounded-full text-base font-bold" disabled={loading}>
          {loading ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" /> Memproses...</> : "PROCEED TO CHECKOUT"}
        </Button>
      </form>
    </div>
  );
